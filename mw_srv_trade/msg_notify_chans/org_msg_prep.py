...
def exit_order_preparation(client_capital_book, inst_record, inst_order_record, sp_user_session, strategy_name):
    ...
    day_inst_orders = pd.read_csv(DAY_INSTRUMENT_ORDERS)
    day_inst_orders[columns] = day_inst_orders[columns].astype(str)

    day_inst_orders = day_inst_orders[
        (day_inst_orders['instrument entry type'] == str(inst_order_record['inst_option_name']))]
    ...
    day_inst_orders.at[day_inst_orders_index, 'instrument exit type'] = day_inst_orders_['instrument entry type']
    day_inst_orders.at[day_inst_orders_index, 'instrument exit qty'] = day_inst_orders_['instrument entry qty']
    day_inst_orders.at[day_inst_orders_index, 'instrument exit time'] = datetime.now().strftime('%H:%M:%S')
    day_inst_orders.at[day_inst_orders_index, 'instrument exit price'] = instrument_exit_price
    day_inst_orders.at[day_inst_orders_index, 'client current capital'] = client_current_capital

    entry_price = day_inst_orders.at[day_inst_orders_index, 'instrument entry price']
    exit_price = day_inst_orders.at[day_inst_orders_index, 'instrument exit price']
    profit_loss = (entry_price - exit_price)
    inst_profit_loss = -(profit_loss * day_inst_orders_['instrument entry qty'])
    day_inst_orders.at[day_inst_orders_index, 'instrument profit or loss'] = inst_profit_loss

    return day_inst_orders, day_inst_orders_index
